        ...
        <div className="space-y-4 py-4">
          ...
          <div className="flex justify-between items-center">
            ...
            <Badge variant={isWin ? "default" : "destructive"} className={cn(
                isWin ? 'bg-green-600' : isLoss ? 'bg-red-600' : 'bg-gray-500', 
                'text-white'
            )}>
              {isWin ? 'Ganancia' : isLoss ? 'Pérdida' : 'Doji' }
            </Badge>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-muted-foreground">Beneficio/Pérdida</span>
            <span className={cn("font-semibold", isWin ? 'text-green-600' : isLoss ? 'text-red-600' : 'text-gray-500')}>
              {formattedProfit}
            </span>
          </div>
          {trade.pips != null && (
