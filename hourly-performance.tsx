...
import { Plus, RotateCcw, Trophy, Skull, Calendar as CalendarIcon, Heart, Minus, ShieldOff, BarChart2, Upload, Download, History, HelpCircle, TimerOff } from 'lucide-react';
...
import TrendPerformance from '@/components/dashboard/trend-performance';
import DailyTradeLimiter from '@/components/dashboard/daily-trade-limiter';
...
export default function DashboardPage() {
  ...
  const [dailyHealth, setDailyHealth] = useState<DailyHealth>({ lives: 4, date: new Date().toISOString() });
  ...
  useEffect(() => {
    loadAllData();
     ...
     const handleStorageChange = (e: StorageEvent) => {
        const keysToWatch = ['trades', 'withdrawals', 'balanceAdditions', 'adjustments', 'playerStats', 'bestiaryCreatures', 'journalEntries', 'xp_updated', 'dailyHealth', 'navItems', 'dailyMission'];
 ...
 const handleAddTrade = (trade: Omit<Trade, 'id'>) => {
    const dailyLimit = JSON.parse(localStorage.getItem('dailyTradeLimit') || '10');
    const todaysTradesCount = trades.filter(t => isSameDay(new Date(t.date), new Date())).length;

    if (todaysTradesCount >= dailyLimit) {
        toast({
            variant: 'destructive',
            title: 'Límite de Operaciones Alcanzado',
            description: `Has alcanzado tu límite de ${dailyLimit} operaciones para hoy.`,
        });
        return;
    }
 ...
 const handleFullReset = () => {
  ...
  const { gains, losses, totalLossesCount, totalXpLost } = useMemo(() => {
    ...
    return { gains, losses, totalLossesCount, totalXpLost };
  ...
  const handleDeleteSelected = () => {
    const newTrades = trades.filter(t => !selectedTrades.includes(t.id));
    setTrades(newTrades);
    localStorage.setItem('trades', JSON.stringify(newTrades));
    ...
    toast({
        title: `${selectedTrades.length} Operaciones Eliminadas`,
        description: "Las operaciones seleccionadas han sido eliminadas.",
        variant: "destructive"
    });
    setSelectedTrades([]);
  }

  const handleToggleTradeSelection = (id: string) => {
  ...
  const handleSelectAll = () => {
    setSelectedTrades(activities.filter(a => a.type === 'trade').map(t => t.id));
  };
  ...
  const handleResetLives = () => {
    const newHealth = { lives: 4, date: new Date().toISOString() };
  ...
  const handleRemoveLife = () => {
    setDailyHealth(prev => {
        ...
        if (newLives === 2) {
            toast({
                title: "Sistema: ¡Cuidado!",
                description: "Ya ha terminado su sesión. Por su protección, finalice con la incursión y espere que todo se normalice. Descanse y cúrese para una siguiente incursión.",
                variant: "destructive",
                duration: 10000,
            });
        } else if (newLives === 0) {
            toast({
                title: "Sistema: Límite Alcanzado",
                description: "Usted llegó a su límite en la incursión. Por la protección de su vida, no puede seguir más. Recupérese, sánese y corrija, que mañana será un día mejor. ¡Usted puede, Cazador!",
                variant: "destructive",
                duration: 10000,
            });
        }
    ...
    reader.onload = (e) => {
        try {
            ...
            const sheetActions: { [key: string]: (data: any[]) => void } = {
                trades: (d) => localStorage.setItem('trades', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('trades') || '[]'), d))),
                withdrawals: (d) => localStorage.setItem('withdrawals', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('withdrawals') || '[]'), d))),
                balanceAdditions: (d) => localStorage.setItem('balanceAdditions', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('balanceAdditions') || '[]'), d))),
                adjustments: (d) => localStorage.setItem('adjustments', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('adjustments') || '[]'), d))),
                creatures: (d) => localStorage.setItem('bestiaryCreatures', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('bestiaryCreatures') || '[]'), d))),
                journalEntries: (d) => localStorage.setItem('journalEntries', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('journalEntries') || '[]'), d))),
                habitTasks: (d) => localStorage.setItem('habitTasks', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('habitTasks') || '[]'), d))),
                tournamentPosts: (d) => localStorage.setItem('tournamentPosts', JSON.stringify(mergeById(JSON.parse(localStorage.getItem('tournamentPosts') || '[]'), d))),
            ...
            if (wb.SheetNames.includes('strategyOptions')) {
                ...
                const existingStrategies = JSON.parse(localStorage.getItem('strategyOptions') || '[]');
                const combined = [...existingStrategies, ...strategies];
                const uniqueStrategies = [...new Set(combined)];
                localStorage.setItem('strategyOptions', JSON.stringify(uniqueStrategies));
            }
            ...
            toast({
                ...
                description: "Tus datos han sido restaurados y fusionados. La página se recargará."
            });
  ...
  const { winningStreak, maxWinningStreak, losingStreak, maxLosingStreak } = useMemo(() => {
    ...
    // Calculate final streaks at the end of the period
    let finalWinningStreak = 0;
    ...
    let finalLosingStreak = 0;
    for (let i = sortedTrades.length - 1; i >= 0; i--) {
        if (sortedTrades[i].status === 'loss') {
            finalLosingStreak++;
        } else {
            break;
        }
    ...
    return { 
        winningStreak: finalWinningStreak, 
        maxWinningStreak,
        losingStreak: finalLosingStreak,
        maxLosingStreak,
    };
  }, [filteredTrades]);
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
          ...
          <main className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 items-start">
                  ...
                  <DailyTradeLimiter trades={filteredTrades} />
                  <DailyMissionCard />
              </div>
              ...
              {showBarCharts && (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <TrendPerformance trades={filteredTrades} />
                  </div>
                </div>
              )}
