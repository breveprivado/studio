...
const DailyLedger = ({ selectedDate }: { selectedDate: Date }) => {
    ...
    const [startDate, setStartDate] = useState(() => {
        ...
        // Default to Sep 13 of the current year
        return new Date(year, 8, 13);
    });
    ...
    const [projectedBalances, setProjectedBalances] = useState<{ [date: string]: number }>({});

    const [editingBalance, setEditingBalance] = useState<{ date: string; value: string } | null>(null);
    const [editingProjected, setEditingProjected] = useState<{ date: string; value: string } | null>(null);
    ...
    useEffect(() => {
        ...
        const storedStartDate = localStorage.getItem('dailyLedger_startDate');
        if(storedStartDate) setStartDate(new Date(JSON.parse(storedStartDate)));
        ...
        const storedProjectedBalances = localStorage.getItem('dailyLedger_projectedBalances');
        if (storedProjectedBalances) setProjectedBalances(JSON.parse(storedProjectedBalances));
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        localStorage.setItem('dailyLedger_startDate', JSON.stringify(startDate));
    }, [startDate]);
     useEffect(() => {
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        localStorage.setItem('dailyLedger_projectedBalances', JSON.stringify(projectedBalances));
    }, [projectedBalances]);
     useEffect(() => {
    ...
    const handleProjectedChange = (date: string, value: string) => {
        setEditingProjected({ date, value });
    };

    const handleProjectedBlur = (date: string) => {
        if (editingProjected && editingProjected.date === date) {
            const newProjectedValue = editingProjected.value;
            setProjectedBalances(prev => {
                const newProjectedBalances = { ...prev };
                if (newProjectedValue === '' || newProjectedValue === null || isNaN(parseFloat(newProjectedValue))) {
                    delete newProjectedBalances[date];
                } else {
                    const newProjected = parseFloat(newProjectedValue);
                    newProjectedBalances[date] = newProjected;
                }
                return newProjectedBalances;
            });
        }
    ...
    const getWeeklyGainForWeek = (weekNumber: number) => {
        ...
        return applicablePhase?.weeklyGain ?? 0;
    };
    ...
    const fullYearData = useMemo(() => {
        ...
        for (let i = 0; i < 365; i++) {
            ...
            // Check for manual override in projected balances for the *previous* day
            const prevDateKey = format(addDays(currentDate, -1), 'yyyy-MM-dd');
            if (projectedBalances[prevDateKey] !== undefined) {
                lastKnownBalance = projectedBalances[prevDateKey];
            } else if (i > 0) {
                 const prevDayData = data[i - 1];
                 if (prevDayData) {
                    lastKnownBalance = prevDayData.projectedBalance;
                 }
            }


            if (dayOfWeek === 1 || i === 0) { // If it's a Monday or the very first day
                 const weeklyGain = getWeeklyGainForWeek(weekNumber);
                 dailyGoalForCurrentWeek = weeklyGain;
            }

            let dailyGoal = 0;
            if(isWeekday){
                dailyGoal = dailyGoalForCurrentWeek;
            }
            ...
            // If there's a manual override for the *current* day, use it.
            if (projectedBalances[dateKey] !== undefined) {
                projectedBalance = projectedBalances[dateKey];
            }
        ...
        return data;
    }, [initialBalance, gainPhases, balances, projectedBalances, startDate]);
    ...
    const { todayProjected, todayActual, performanceDifference, chartData, percentageValue } = useMemo(() => {
        ...
        const lastActualBalanceEntry = balancesWithValues.length > 0 ? balancesWithValues[0] : null;
        
        let lastActualBalance = initialBalance;
        let performanceDiff = 0;
        
        if(lastActualBalanceEntry) {
            lastActualBalance = lastActualBalanceEntry.balance;
            const previousBalanceEntry = balancesWithValues.find(item => item.date < lastActualBalanceEntry.date);
            const previousBalance = previousBalanceEntry?.balance ?? initialBalance;
            performanceDiff = lastActualBalance - previousBalance;
        }
    ...
    const tableData = useMemo(() => {
        return fullYearData.map((dayData, index) => {
            const actualBalance = dayData.actualBalance;
            let difference;

            if (actualBalance !== undefined) {
                // Find previous day with an actual balance
                let previousActualBalance = initialBalance;
                for (let i = index - 1; i >= 0; i--) {
                    const prevDayActual = fullYearData[i].actualBalance;
                    if (prevDayActual !== undefined) {
                        previousActualBalance = prevDayActual;
                        break;
                    }
                }
                difference = actualBalance - previousActualBalance;
            }
                ...
                <AccordionContent>
                    <div className="space-y-6">
                        <div className="space-y-4 p-4 border rounded-lg">
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                 ...
                                 <div>
                                    <Label>Fecha de Inicio de Proyección</Label>
                                    <Popover>
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <Card className={cn(performanceDifference >= 0 ? "bg-green-50 dark:bg-green-900/20" : "bg-red-50 dark:bg-red-900/20")}>
                                <CardHeader className="pb-2">
                                    <CardDescription>Rendimiento Diario Real</CardDescription>
                         ...
                         <Card>
                            <CardHeader>
                                <CardTitle>Gráfico de Proyección</CardTitle>
                        ...
                        <Accordion type="single" collapsible className="w-full">
                            <AccordionItem value="road-to-olimpo">
                                <AccordionTrigger>
                                    <CardHeader className="p-0 text-left">
                                        ...
                                        <CardDescription>Tu proyección de crecimiento día a día.</CardDescription>
                                    </CardHeader>
                                </AccordionTrigger>
                                <AccordionContent>
                                    <CardContent className="max-h-[500px] overflow-y-auto px-0">
                                        <Table>
                                            ...
                                            <TableBody>
                                                {tableData.map(({ date, dateKey, weekNumber, dailyGoal, projectedBalance, actualBalance, difference }, index) => {
                                                    const isNewWeek = date.getDay() === 1 || index === 0;
                                                    const finalProjectedBalance = projectedBalances[dateKey] ?? projectedBalance;
                                                        ...
                                                        <React.Fragment key={dateKey}>
                                                            ...
                                                            <TableRow className={cn(isSameDay(date, selectedDate) && "bg-muted/50")}>
                                                                ...
                                                                <TableCell>
                                                                    <Input
                                                                        type="text"
                                                                        value={editingProjected?.date === dateKey ? editingProjected.value : formatCurrency(finalProjectedBalance)}
                                                                        onChange={(e) => handleProjectedChange(dateKey, e.target.value.replace(/[^0-9.-]+/g,""))}
                                                                        onFocus={(e) => e.target.select()}
                                                                        onBlur={() => handleProjectedBlur(dateKey)}
                                                                        placeholder="-"
                                                                        className="h-8 w-32"
                                                                    />
                                                                </TableCell>
                                                                <TableCell>
                                                                    <Input
                                                                        type="text"
                                                                        value={editingBalance?.date === dateKey ? editingBalance.value : (actualBalance?.toString() ?? '')}
                                                                        onChange={(e) => handleBalanceChange(dateKey, e.target.value.replace(/[^0-9.-]+/g,""))}
...
export default function JournalPage() {
  ...
  const [editingCompletedRuleIds, setEditingCompletedRuleIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    if (entryForSelectedDate) {
      ...
      setCompletedRuleIds(entryForSelectedDate.completedRuleIds || []);
    } else {
      setCurrentEntry('');
      setCurrentRating(0);
 ...
 const checkSurvivalMissions = (newEntriesList: JournalEntry[], newRating: number) => {
  ...
  const handleEditEntry = (entry: JournalEntry) => {
    ...
    setEditingCompletedRuleIds(entry.completedRuleIds || []);
  };
  ...
  const handleUpdateEntry = () => {
    ...
    const newEntries = entries.map(entry =>
      entry.id === editingEntryId
        ? { 
            ...
            completedRuleIds: editingCompletedRuleIds
          }
        : entry
    );
    setEntries(newEntries);
    ...
    if(updatedEntry && isSameDay(new Date(updatedEntry.date), selectedDate)) {
        ...
        setCompletedRuleIds(editingCompletedRuleIds);
    }
  ...
  const handleCompletedRuleChange = (ruleId: string, isCompleted: boolean) => {
    if (editingEntryId) {
        setEditingCompletedRuleIds(prev => 
            isCompleted ? [...prev, ruleId] : prev.filter(id => id !== ruleId)
        );
    } else {
